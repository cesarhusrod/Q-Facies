...
class Skeleton:
    ...
    def __init__(self):
        ...
        self.tri_params = {'facecolor': 'None',
                           'lw': 0.5, 'ec': 'black', 'zorder': 3}
        # --------------------------------------------------------------------
        [self.ax.add_patch(i) for i in (
            self._cation(), self._anion(), self._diamond())]
        self._grid()
        self._labels()
    ...
    def _grid(self):
        ...
        [self.ax.plot([((100 + offset) + espaciado) * cos, (((100 + offset) + espaciado) + 100) * cos],
                      [(sin * (100 + offset + espaciado)),
                       sin * (offset + espaciado)],
                      **gl_params)
         for espaciado in range(20, 100, 20)]
    ...
    def _labels(self):
        ...
        text_params = {'va': 'center', 'ha': 'center',
                       'style': 'italic', 'fontsize': 5}
        ...
        self.ax.text(
            x_txt, y_txt, '$SO_{4}^{2-}$ + $Cl^-$', rotation=60, **text_params)
        ...
        self.ax.text(
            x_txt, y_txt, '$CO_{3}^{2-}$ + $HCO_{3}^{-}$', rotation=60, **text_params)
        ...
        self.ax.text(
            x_txt, y_txt, '$Na^{+} + K^{+}$', rotation=300, **text_params)
        ...
        self.ax.text(
            x_txt, y_txt, '$Ca^{2+} + Mg^{2+}$', rotation=300, **text_params)
        ######################### -------- SO4 ------ #########################
        x = 100 + offset + np.linspace(50, 100, 9)[6]
        y = tan * x_array[2] + l_offset
        self.ax.arrow(x, y, -cos * 25, sin * 25, **arrow_params)
        x_txt, y_txt = 149 + offset + x_array[6], sin * 18 + y
        self.ax.text(x_txt, y_txt, '$SO_{4}^{2-}$',
                     rotation=300, **text_params)
        # h:                 y = 0
        ######################### -------- Ca ------ ##########################
...
class Overlap(Skeleton):
    ...
    def extract_centroid(self):
        ...
        if self.overlap_var == 'Time':
            values = [mdates.date2num(i.__str__()).mean()
                      for i in self.diagrams]
    ...
    def colorbar(self, overfig_way):
        ...
        if self.overlap_var != 'Time':
            ...
            ticklabels = [mdates.num2date(i).strftime(
                "%d-%m-%Y") for i in ticks]
    ...
    def layout(self):
        ...
        text = "Graduation of all groups' convex-hulls \naccording to {}{}. {}"\
               .format(r'${}$'.format(self.overlap_var),
...
class Plot_Diagram(Skeleton):
    ...
    def __init__(self, panels, name, way, df_name,
        ...
        self.ch_points()
        self.facies()
        self.lines(clip=kw['clip_lines'])
        self.info()
        if (kw['plot_outliers'] & kw['lof']):
            self.outliers()
        self.layout()
    ...
    def points(self):
        ...
        points = np.concatenate(
            tuple(i.Params.points for i in self.panels), axis=0)
    ...
    def outliers(self):
        ...
        kw = {'s': self.kw_format['point_size'],
              ...
              'lw': 0,
              'zorder': 3}
        points = np.concatenate(tuple(panel.Params.outliers
                                      for panel in self.panels), axis=0)
        scatter = self.ax.scatter(points[:, 0], points[:, 1], **kw)
        scatter.set_clip_path(self.get_clip_path())
    ...
    def ch_points(self):
        ...
        kw = {'s': self.kw_format['point_size'],
              ...
              'zorder': 4}
        points = np.concatenate(
            tuple(i.Params.ch_points for i in self.panels), axis=0)
        scatter = self.ax.scatter(points[:, 0], points[:, 1], **kw)
        scatter.set_clip_path(self.get_clip_path())
...
class Evolution:
    ...
    def __init__(self, diagrams, df, df_name, way, mapa_dict):
        ...
        self.fig, self.ax = plt.subplots(
            3, figsize=(20, 17), sharex=True, sharey=True)
    ...
    def layout(self):
        ...
        [i.tick_params(axis='x', bottom=False)
         for i in (self.ax[0], self.ax[1])]
        # Labelsize of all labels
        [j.tick_params(axis='both', labelsize=20)
         for i in self.axis for j in i]

        # Axis titles
        [i.set_title(r'${}$'.format(j), fontdict=dict(fontsize=18, ha='center'),
...
class Evolution_time(Evolution):
    ...
    def _format_for_time(self):
        ...
        self.df['Onset'] = self.df.Group.map(
            lambda x: pd.to_datetime(x.split(' to ')[0]))
        self.df['End'] = self.df.Group.map(
            lambda x: pd.to_datetime(x.split(' to ')[1]))
        self.df['Time'] = self.df['Onset'] + \
            (self.df['End'] - self.df['Onset'])/2
    ...
    def _lines(self, var, **kw):
        ...
        # For plotting zoom ts (x100).
        kw_com_x100 = dict(style='-.', **kw_com)

        # Pivot the df by certain variable and sort groups.
        ts = pd.pivot_table(self.df, values=var,
                            index='Time', columns=['Panel'])
    ...
    def _ticks(self, way):
        ...
        [i.set_ylabel('%', fontsize=20, rotation=180,
                      ha='left', va='center') for i in ax]
        [i.set_ylabel('Degrees', fontsize=20, rotation=270,
                      ha='center', va='baseline') for i in axt]
